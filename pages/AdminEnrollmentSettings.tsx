    ...
    <div className="flex h-screen overflow-hidden bg-[#f5f5f5]">
      ...
      {toast ? (
        <div
          className={`fixed right-4 top-4 z-50 rounded-[6px] px-4 py-3 text-sm font-medium ${
            toast.type === "success" ? "bg-[#eafaf1] text-[#27ae60]" : "bg-[#fdecea] text-[#e74c3c]"
          }`}
        >
          {toast.message}
        </div>
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <section className="mb-6">
            <button
              ...
              className="mb-3 rounded-[6px] border border-[#077d8a] bg-white px-4 py-2 text-sm font-medium text-[#077d8a] hover:bg-[#e0e0e0]"
            >
              Back
            </button>
            <h1 className="text-2xl font-semibold text-[#333333]">Enrollment Settings</h1>
            <p className="mt-1 text-base text-[#666666]">Singleton config for enrollment control</p>
          </section>

          <section className="rounded-[8px] border border-[#cccccc] bg-white p-5 shadow-[0_2px_6px_rgba(0,0,0,0.1)]">
            <h2 className="text-lg font-semibold text-[#333333]">Current Setting</h2>

            {loadingCurrent ? <p className="mt-4 text-sm text-[#666666]">Loading current setting...</p> : null}
            {!loadingCurrent && fetchError ? (
              <div className="mt-4 rounded-[6px] bg-[#fdecea] px-3 py-2 text-sm text-[#e74c3c]">{fetchError}</div>
            ) : null}
            {!loadingCurrent && notFound ? (
              <div className="mt-4 rounded-[6px] bg-[#eafaf1] px-3 py-2 text-sm text-[#077d8a]">
            ...
            {!loadingCurrent && setting ? (
              <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-3">
                <div className="rounded-[6px] border border-[#cccccc] bg-[#f5f5f5] p-3">
                  <p className="text-sm text-[#666666]">Status</p>
                  <span
                    className={`mt-1 inline-flex rounded-full px-3 py-1 text-xs font-semibold ${
                      setting.is_active ? "bg-[#eafaf1] text-[#27ae60]" : "bg-[#fdecea] text-[#e74c3c]"
                    }`}
                  >
                    {setting.is_active ? "Open" : "Closed"}
                  </span>
                  <p className="mt-3 text-sm text-[#666666]">Remaining Time</p>
                  <p className="text-sm font-semibold text-[#333333]">{openRemaining}</p>
                </div>
                <div className="rounded-[6px] border border-[#cccccc] bg-[#f5f5f5] p-3">
                  <p className="text-sm text-[#666666]">Enrollment Open At</p>
                  <p className="text-sm font-semibold text-[#333333]">{formatDateTime(setting.enrollment_open_at)}</p>
                  <p className="mt-3 text-sm text-[#666666]">Enrollment Close At</p>
                  <p className="text-sm font-semibold text-[#333333]">{formatDateTime(setting.enrollment_close_at)}</p>
                </div>
                <div className="rounded-[6px] border border-[#cccccc] bg-[#f5f5f5] p-3">
                  <p className="text-sm text-[#666666]">Max Credits</p>
                  <p className="text-sm font-semibold text-[#333333]">{setting.max_credits}</p>
                  <p className="mt-2 text-sm text-[#666666]">Max Courses</p>
                  <p className="text-sm font-semibold text-[#333333]">{setting.max_courses ?? "N/A"}</p>
                  <p className="mt-2 text-sm text-[#666666]">Allow Waitlist</p>
                  <p className="text-sm font-semibold text-[#333333]">{setting.allow_waitlist ? "Yes" : "No"}</p>
                </div>
              </div>
            ) : null}
          </section>

          <section className="mt-6 rounded-[8px] border border-[#cccccc] bg-white p-5 shadow-[0_2px_6px_rgba(0,0,0,0.1)]">
            <h3 className="text-lg font-semibold text-[#333333]">Enrollment Setting Form</h3>
            ...
            >
              <div>
                <label className="mb-2 block text-[14px] font-medium text-[#333333]">Duration Type</label>
                <div className="inline-flex rounded-[6px] border border-[#cccccc] bg-white p-1">
                  <button
                    type="button"
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                  <label className="mb-1 block text-[14px] font-medium text-[#333333]">
                  ...
                  <input
                    ...
                    className="w-full rounded-[6px] border border-[#cccccc] bg-white p-[10px_12px] text-[#333333] outline-none focus:border-[#077d8a] focus:shadow-[0_0_0_2px_rgba(7,125,138,0.2)]"
                  />
                ...
                <div>
                  <label className="mb-1 block text-[14px] font-medium text-[#333333]">Max Credits</label>
                  <input
                    type="number"
                    min={1}
                ...
                <div>
                  <label className="mb-1 block text-[14px] font-medium text-[#333333]">Max Courses (optional)</label>
                  <input
                    type="number"
                    min={1}
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <label className="flex items-center justify-between rounded-[6px] border border-[#cccccc] p-3">
                  <span className="text-sm font-medium text-[#333333]">Allow waitlist</span>
                  <input
                    type="checkbox"
                ...
                <label className="flex items-center justify-between rounded-[6px] border border-[#cccccc] p-3">
                  <span className="text-sm font-medium text-[#333333]">Is active</span>
                  <input
                    type="checkbox"
              ...
              {formApiError ? (
                <div className="rounded-[6px] bg-[#fdecea] px-3 py-2 text-sm text-[#e74c3c]">{formApiError}</div>
              ) : null}
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <button
                  ...
                  className="rounded-[6px] border border-[#077d8a] bg-white px-6 py-3 text-sm font-medium uppercase text-[#077d8a] hover:bg-[#e0e0e0] disabled:cursor-not-allowed disabled:opacity-60"
                >
          ...
          <section className="mt-6 rounded-[8px] border border-[#cccccc] bg-white p-5 shadow-[0_2px_6px_rgba(0,0,0,0.1)]">
            <h3 className="text-lg font-semibold text-[#333333]">Status Actions</h3>
            <p className="mt-1 text-sm text-[#666666]">Open or close enrollment with confirmation.</p>
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              <button
                ...
                className="rounded-[6px] border border-[#077d8a] bg-white px-5 py-3 text-sm font-medium text-[#077d8a] hover:bg-[#e0e0e0] disabled:cursor-not-allowed disabled:opacity-50"
              >
                Close Enrollment
              </button>
            </div>
      ...
      {confirmStatus ? (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-[8px] bg-white p-5 shadow-lg">
            <h4 className="text-lg font-semibold text-[#333333]">Confirm Status Change</h4>
            ...
            <div className="mt-5 flex justify-end gap-3">
              <button
                ...
                className="rounded-[6px] border border-[#077d8a] bg-white px-4 py-2 text-sm font-medium text-[#077d8a] hover:bg-[#e0e0e0]"
              >
                Cancel
              </button>
              <button
