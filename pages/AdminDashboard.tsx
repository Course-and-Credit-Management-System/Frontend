...
const AdminDashboard: React.FC<DashboardProps> = ({ user, onLogout }) => {
  ...
  const majorStats = [
    { code: 'SE', name: 'Software Engineering', count: 342, color: 'bg-blue-500' },
    { code: 'KE', name: 'Knowledge Engineering', count: 215, color: 'bg-indigo-500' },
    { code: 'BIS', name: 'Business Info Systems', count: 184, color: 'bg-emerald-500' },
    { code: 'ES', name: 'Embedded Systems', count: 142, color: 'bg-amber-500' },
    { code: 'NET', name: 'Networking', count: 165, color: 'bg-cyan-500' },
    { code: 'SEC', name: 'Cyber Security', count: 198, color: 'bg-rose-500' },
    { code: 'HPC', name: 'High Perf. Computing', count: 86, color: 'bg-fuchsia-500' },
  ];
  
  const maxCount = Math.max(...majorStats.map(m => m.count));
            ...
            <div className="group relative overflow-hidden rounded-xl bg-surface-light p-6 shadow-sm transition-all hover:shadow-md dark:bg-surface-dark">
              ...
              <div className="relative z-10 flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Register individual student profiles directly.</p>
                </div>
            ...
            <div className="group relative overflow-hidden rounded-xl bg-surface-light p-6 shadow-sm transition-all hover:shadow-md dark:bg-surface-dark">
              ...
              <div className="relative z-10 flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Import student lists or exam results from CSV/XLS.</p>
                </div>
            ...
            <div className="group relative overflow-hidden rounded-xl bg-surface-light p-6 shadow-sm transition-all hover:shadow-md dark:bg-surface-dark">
              ...
              <div className="relative z-10 flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Review, grade, and publish semester results.</p>
                </div>
            ...
            <div className="rounded-xl bg-surface-light shadow-sm dark:bg-surface-dark lg:col-span-8 p-6">
              <div className="flex items-center justify-between mb-6">
                 <div>
                   ...
                   <p className="text-sm text-gray-500 dark:text-gray-400">Distribution by status and major</p>
                 </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                 <div className="bg-green-50 dark:bg-green-900/10 border border-green-100 dark:border-green-900/30 p-5 rounded-xl flex items-center gap-5 transition-transform hover:scale-[1.01]">
                    ...
                    <div>
                        ...
                        <h3 className="text-3xl font-bold text-gray-800 dark:text-white">1,245</h3>
                ...
                <div className="bg-orange-50 dark:bg-orange-900/10 border border-orange-100 dark:border-orange-900/30 p-5 rounded-xl flex items-center gap-5 transition-transform hover:scale-[1.01]">
                    ...
                    <div>
                        ...
                        <h3 className="text-3xl font-bold text-gray-800 dark:text-white">84</h3>
              ...
              <div className="bg-gray-50 dark:bg-gray-800/30 rounded-xl p-6 border border-gray-100 dark:border-gray-700">
                 <h4 className="text-sm font-bold text-gray-700 dark:text-gray-300 mb-6 uppercase tracking-wider">Students by Major</h4>
                 ...
                 <div className="w-full h-64 flex items-end justify-between gap-2 sm:gap-4 relative z-0">
                   ...
                   <div className="absolute inset-0 flex flex-col justify-between pointer-events-none z-[-1]">
                       {[0, 25, 50, 75, 100].map((tick, i) => (
                           <div key={i} className="w-full border-b border-gray-200 dark:border-gray-700 h-0 border-dashed opacity-50 relative">
                               <span className="absolute -top-2.5 left-0 text-[10px] text-gray-400">{Math.round((1 - i/4) * maxCount)}</span>
                           </div>
                       ))}
                   </div>
                   ...
                   {majorStats.map((item) => {
                       ...
                       return (
                           ...
                           >
                               {/* Tooltip */}
                               <div className={`absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-xs rounded px-2.5 py-1.5 transition-all z-20 whitespace-nowrap shadow-lg pointer-events-none transform ${
                               ...
                               <div 
                                  className={`w-full max-w-[40px] sm:max-w-[50px] rounded-t-lg transition-all duration-300 relative ${item.color} ${hoveredMajor === item.code ? 'opacity-100 scale-y-105' : 'opacity-85'}`}
                                  style={{ height: `${heightPercent}%` }}
                               >
                               </div>
                               <span className="text-[10px] sm:text-xs text-gray-600 dark:text-gray-400 mt-3 font-bold truncate w-full text-center">{item.code}</span>
                           </div>
                       )
                   })}
                 </div>
            ...
            <div className="flex flex-col gap-6 lg:col-span-4">
              <div className="rounded-xl bg-surface-light p-6 shadow-sm dark:bg-surface-dark">
                ...
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <span className="rounded bg-green-100 px-2 py-1 text-xs font-bold uppercase text-green-800 dark:bg-green-900/30 dark:text-green-400">Active</span>
                </div>
              ...
              <div className="flex-1 rounded-xl bg-surface-light p-6 shadow-sm dark:bg-surface-dark">
                ...
                <div className="space-y-4">
                  {[
                    { title: 'Major Changes', time: '2m ago', desc: '5 students requested major changes requiring dean approval.', icon: 'warning', iconColor: 'text-orange-500' },
                    { title: 'Credit Overload', time: '1h ago', desc: '12 students applied for extra credits this semester.', icon: 'school', iconColor: 'text-cyan-600' }
                  ].map((item, i) => (
                    <div key={i} className="flex items-start rounded-lg border border-border-light bg-gray-50 p-3 dark:border-border-dark dark:bg-slate-800/50">
