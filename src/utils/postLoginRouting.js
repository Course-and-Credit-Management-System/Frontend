...
const sortRegistrationsByRecent = (items) =>
...
const hasSectionContent = (payload) => {
...
export async function detectStudentDetailsRecord(api, studentSession) {
...
export function deriveAcademicProgress(startRoutePayload) {
...
export function decidePostLoginRoute(studentSession, studentDetails, academicProgress) {
  ...
  /*
    Test cases:
    1) NEW student (details missing):
       decidePostLoginRoute(session, { exists: false }, anyProgress)
       => { type: "REDIRECT", path: "/student-details" }

    2) OLD student after 1 semester:
       decidePostLoginRoute(session, { exists: true }, { eligibilityKnown: true, enrollmentEligible: true })
       => { type: "CHOICE", options: [Update Student Details, Enroll] } (Enroll enabled)

    3) OLD student after 2 semesters (year changed):
       decidePostLoginRoute(session, { exists: true }, { reason: "YEAR_CHANGED", eligibilityKnown: true, enrollmentEligible: false })
       => { type: "CHOICE", options: [Update Student Details, Enroll] } (Enroll disabled)
  */
  return {
    type: 'CHOICE',
    options: [
